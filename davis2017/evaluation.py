...
from davis2017.results import Results, Results_SepObjs
...
class DAVISEvaluation(object):
    ...
    def _evaluate_unsupervised(all_gt_masks, all_res_masks, all_void_masks, metric, max_n_proposals=20):
        ...
        for ii in range(all_gt_masks.shape[0]):
            for jj in range(all_res_masks.shape[0]):
                if 'J' in metric:
                    ...
                    #print("<><><><><><<<<<<<<<<<<<<<>>>>>>>>>>", j_metrics_res[jj, ii, :].shape, j_metrics_res[jj, ii, :])
                if 'F' in metric:
        ...
        print("j_metrics_res :", j_metrics_res.shape)
        print("f_metric_res :", f_metrics_res.shape)
        #print(j_metrics_res[...,0])
        #print(f_metrics_res[...,0])
    ...
    def evaluate(self, res_path, metric=('J', 'F'), debug=False):
        ...
        '''for seq in tqdm(list(self.dataset.get_sequences())):
            ...
            print(f'{seq} : {all_gt_masks.shape} {all_void_masks.shape} {len(all_masks_id)}')'''
        

        # Sweep all sequences
        #results = Results(root_dir=res_path)
        results = Results_SepObjs(root_dir=res_path)
        for seq in tqdm(list(self.dataset.get_sequences())):
            ...
            print(f'{seq} : {all_gt_masks.shape} {all_res_masks.shape} {all_void_masks.shape} {len(all_masks_id)}')

            '''if seq == 'india' :
                all_gt_masks = all_gt_masks[0:3,...]'''
            print(f'--{seq} : {all_gt_masks.shape} {all_res_masks.shape} {all_void_masks.shape} {len(all_masks_id)}')


            if self.task == 'unsupervised':
                print("eval unsupervised")
...
class DAVISEvaluationOld(object):
    def __init__(self, davis_root, task, gt_set, sequences='all', codalab=False):
        """
        Class to evaluate DAVIS sequences from a certain set and for a certain task
        :param davis_root: Path to the DAVIS folder that contains JPEGImages, Annotations, etc. folders.
