...
class Results_SepObjs(object):
    def __init__(self, root_dir):
        self.root_dir = root_dir

    def _read_mask(self, sequence, frame_id):
        try:
            mask_path = os.path.join(self.root_dir, sequence, '1', f'{frame_id}.png')
            return np.array(Image.open(mask_path))
        except IOError as err:
            sys.stdout.write(sequence + " frame %s not found!\n" % frame_id)
            sys.stdout.write("The frames have to be indexed PNG files placed inside the corespondent sequence "
    ...
    def _read_mask_all_objs(self, sequence, frame_id):

        num_objs = len(os.listdir(os.path.join(self.root_dir, sequence)))
        ...
        for obj_id in range(1,num_objs+1):
            ...
            try:
                mask_path = os.path.join(self.root_dir, sequence, str(obj_id), f'{frame_id}.png')
                all_masks.append(np.array(Image.open(mask_path)))
            except IOError as err:
                sys.stdout.write(sequence + " frame %s not found!\n" % frame_id)
                sys.stdout.write("The frames have to be indexed PNG files placed inside the corespondent sequence "
                                "folder.\nThe indexes have to match with the initial frame.\n")
    ...
    def read_masks(self, sequence, masks_id):
        ...
        num_objs = len(os.listdir(os.path.join(self.root_dir, sequence)))
        masks = np.zeros((len(masks_id), num_objs, *mask_0.shape))

        for ii, m in enumerate(masks_id):
            masks[ii, ...] = self._read_mask_all_objs(sequence, m)
