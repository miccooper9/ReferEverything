...
def evaluate(args, model) :
    ...
    with open(f'./val_vid2classes.json') as f :
        ...
        for video in tqdm(video_list):
            ...
            for i in range(len(cur_stuff_classes)) :
                ...
                for clip_id in range(0, video_len, num_clip_frames):
                    ...
                    print(f"img shape {imgs.shape} {video_len}")
                ...
                anno_save_path_pred_orig = os.path.join(save_path_prefix, video, class_name, "pred_orig")
                ...
                for f in tqdm(range(vid_masks.shape[0])):
                    ...
                    fig.savefig(os.path.join(anno_save_path_pred_orig, f'{f_id}.png'))
                    plt.close(fig)
...
def main(args):
    ...
    n_parameters = sum(p.numel() for p in model.backbone.unet.parameters())
