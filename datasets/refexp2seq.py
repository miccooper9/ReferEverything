...
class ModulatedDetection(torchvision.datasets.CocoDetection):
    ...
    def __getitem__(self, idx):
        ...
        #print("msk :", np.unique(numpy_masks[0]), numpy_masks[0].dtype)
        msk = Image.fromarray(numpy_masks[0], mode="P")

        im, msk = self._transforms(im, msk)

        #print("-1 im : ",im.shape, im.max(), im.min())
        #print("-1 mask : ",msk.shape, msk.max(), msk.min(), msk.unique())
        ...
        for t in range(self.num_frames - 1):
            ...
            mask_trafo = Image.fromarray(np.concatenate(instance_masks_trafo, axis=0), mode="P")
            
            im_trafo, mask_trafo = self._transforms(im_trafo, mask_trafo)

            #print(t, "im : ",im_trafo.shape, im_trafo.max(), im_trafo.min())
            #print(t, "mask : ",mask_trafo.shape, mask_trafo.max(), mask_trafo.min(), mask_trafo.unique())
            ...
            seq_instance_masks.append(mask_trafo)
        ...
        #print("frames: ", frames.shape, frames.max(), frames.min())
        #print("output_inst_masks: ", output_inst_masks.shape, output_inst_masks.unique())
...
im_transformcog = Tr.Compose([
    Tr.Resize(384,384),
    Tr.ToTensor(),
    Tr.Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5])
])

im_transform_256 = Tr.Compose([
    Tr.Resize(256,256),
    Tr.ToTensor(),
    Tr.Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5])
])
